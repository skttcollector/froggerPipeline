...
def set_project_UI(*args):
	...
	cmds.separator(h=10)
	widgets["frog"] = cmds.button(l="Frogger ML Project", w=300, h=40, bgc=(.7, .7, .5), c=partial(set_project_env_variables, "Frogger"))
	widgets["fit"] = cmds.button(l="Fit and Setup ML Project", w=300, h=40, bgc=(.5, .7, .7), c=partial(set_project_env_variables, "FitAndSetup"))
...
def set_project_env_variables(proj = None, *args):
	...
	ARGS:
		proj (string) - the shortcut for the project we're using. Currently only "frog" and "fit", which stand for Frogger and FitAndSetup
