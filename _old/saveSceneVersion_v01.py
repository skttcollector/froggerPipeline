...
def run(*args, **kwargs):
    ...
    assetNm=__getAssetName(fileNm, padding)
    fileType=__getFileType(fileNm)
    scenesDir="%s%s"%(filePath.split(scenes)[0],scenes)
    versionsPath=__checkVersionsPath(scenesDir, assetNm)
    version=__getVersionNum(versionsPath, assetNm, padding)
    __versionUp(scenesDir, filePath, versionsPath, assetNm, version, fileType)
